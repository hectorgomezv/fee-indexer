...
export class EVMIndexerController {
  ...
  async getFeesCollected(req: Request, res: Response) {
    const { integrator } = req.params;
    const events = await this.service.getFeesCollectedByIntegrator(integrator);
    res.json({
      count: events.length,
      data: events.map((event) => FeesCollectedApiMapper.toApiResponse(event)),
    });
  }
}
